create or replace view rdr.rrv_combination_check_rule (
  rule_set,
  rule,
  condition_id,
  attribute_name,
...
select gl1.lk_match_key1 as	rule_set,
  ...
  left join fdr.fr_general_lookup gl2 on gl1.lk_match_key2 = gl2.lk_match_key1
                                     and gl2.lk_lkt_lookup_type_code = 'COMBO_CHECK'
 where gl1.lk_lkt_lookup_type_code = 'COMBO_RULESET';
comment on table rdr.rrv_combination_check_rule is 'This view displays all the combination checking rules that are currently active within the system such that it can be reported on from the Reporting Data Repository.';