...
/* Formatted on 10/02/2024 4:27:37 PM (QP5 v5.252.13127.32847) */
CREATE OR REPLACE FORCE VIEW RDR.RCV_GLINT_JOURNAL_LINE
(
   JL_JRNL_HDR_ID,
   MAN_JE_ID,
   JL_JRNL_LINE_NUMBER,
         ...
         jl_jrnl_hdr_id,
          ...
          CASE
             WHEN     JL_SEGMENT_1 IN ('UKGAAP_ADJ', 'EURGAAPADJ')
                  AND JL_ATTRIBUTE_4 IN ('FV_UPR_INITIAL',
                                         'FV_UPR_WRITTEN',
                                         'FV_UPR_CHANGE')
             THEN
          ...
          CASE
             WHEN     JL_SEGMENT_1 IN ('UKGAAP_ADJ', 'EURGAAPADJ')
                  AND JL_ATTRIBUTE_4 IN ('FV_UPR_INITIAL',
                                         'FV_UPR_WRITTEN',
                                         'FV_UPR_CHANGE')
                  AND jl_local_amount < 0
             THEN
                ROUND (jl_local_amount, 2)
             WHEN jl_tran_amount < 0
             THEN
                ROUND (jl_tran_amount, 2)
             ELSE
                0
                  ...
                  (SELECT NULL
                    ...
                    WHERE     fgl.lk_lookup_value3 = fgl2.lk_match_key1
                          AND fgl2.lk_lkt_lookup_type_code =
                                 'EVENT_CLASS_PERIOD'
                          AND jl.jl_effective_date <=
                                 (SELECT MIN (period_end)
                                    FROM stn.period_status)));
...
GRANT SELECT ON RDR.RCV_GLINT_JOURNAL_LINE TO AAH_RDR;

GRANT SELECT ON RDR.RCV_GLINT_JOURNAL_LINE TO AAH_READ_ONLY;
