CREATE OR REPLACE PACKAGE BODY GUI."PGUI_MANUAL_JOURNAL" AS
    ...
    ******************************************************************************************/
    ...
    FUNCTION fnui_check_deletion(journal_id NUMBER)									   		    RETURN BOOLEAN;
    FUNCTION fnui_check_copy_line(journal_id NUMBER, line_number NUMBER)			   		    RETURN BOOLEAN;
    FUNCTION fnui_check_delete_line(journal_id NUMBER, line_number NUMBER)			   		    RETURN BOOLEAN;
    ...
    FUNCTION fnui_does_journal_exist(journal_id NUMBER) 							   		    RETURN BOOLEAN;
    FUNCTION fnui_does_line_exist(journal_id NUMBER, line_no NUMBER) 				   		    RETURN BOOLEAN;
    ...
    FUNCTION fnui_create_copy_journal(original_jrnl_id NUMBER)						   		    RETURN BOOLEAN;
    FUNCTION fnui_create_reversing_journal(orignal_jrnl_id NUMBER, reversing_date DATE, entity_proc_group VARCHAR2, status CHAR) RETURN BOOLEAN;
    FUNCTION fnui_check_calendar                                                                RETURN BOOLEAN;
    FUNCTION fnui_get_reversing_journal( journal_id IN NUMBER)					  			    RETURN NUMBER;
    FUNCTION fnui_get_adjustment_journal( journal_id IN NUMBER)								    RETURN NUMBER;
    ...
    ******************************************************************************************/
    ...
    BEGIN
        ...
        IF journal_id IS NOT NULL THEN
           lv_sql_statement := lv_sql_statement || ' AND '||lv_head_prefix||'_jrnl_id = '||TO_CHAR(journal_id);
        END IF;
    ...
    IS
          ...
          CURSOR cur_meta_data IS
            SELECT COLUMN_SCREEN_LABEL,
                   COLUMN_NAME,
                   NVL(FRL.LK_LOOKUP_VALUE1,COLUMN_USED_IN_BALANCE) COLUMN_USED_IN_BALANCE,
              ...
              LEFT OUTER JOIN fdr.fr_general_lookup frl on COLUMN_NAME = FRL.LK_MATCH_KEY2 and FRL.LK_LKT_LOOKUP_TYPE_CODE = 'MADJ_BALANCE_COLUMNS'
...
/*-----------UPGRADE 20.1.1 MERGE END-----------*/
    ...
    IS
      ...
      CURSOR cur_journal_lines(jrnl_id NUMBER) IS
          SELECT jlu_jrnl_line_number
          FROM 	 gui_jrnl_lines_unposted
...
/*-----------UPGRADE 20.1.1 MERGE END-----------*/
    ...
    BEGIN
      ...
      vSql :=
       ...
       SELECT
          STANDARD_HASH(to_char(gjhu.jhu_jrnl_id), ''MD5''),
        ...
        'INSERT INTO slr_jrnl_lines_unposted (
            ...
            jlu_amended_on, jlu_epg_id, jlu_period_month, jlu_period_year, jlu_period_ltd)
        SELECT
            STANDARD_HASH(to_char(gjlu.jlu_jrnl_hdr_id), ''MD5''),
     ...
     loop
        ...
        SELECT
        STANDARD_HASH(to_char(jrnl_file_rec.jf_jrnl_hdr_id), 'MD5'),
         v_file_id
        FROM DUAL;
  ...
  PROCEDURE prui_create_reversing_journal(epg_id in varchar2, journal_id_list in varchar2, process_id NUMBER, status in char)
   ...
   BEGIN
    ...
    WHERE sjhu.jhu_jrnl_id in '||fnui_get_standard_hash_sql (journal_id_list)||'
		 ...
		 EXECUTE IMMEDIATE
		  'UPDATE (select jhu_jrnl_status, jhu_jrnl_status_text
			FROM slr_jrnl_headers_unposted,
				 slr.slr_ext_jrnl_types ejt,
				 slr.slr_jrnl_types jt
			WHERE jhu_jrnl_id in '||fnui_get_standard_hash_sql (journal_id_list)||'
   ...
   BEGIN
     ...
     IF NOT rollback_all THEN
        vSql := vSql||' and exists (select 1 from slr_jrnl_headers_unposted sjhu
          where sjhu.jhu_jrnl_id in '||fnui_get_standard_hash_sql (journal_id_list)||'
          and sjhu.jhu_jrnl_id = standard_hash(to_char(gjhu.jhu_jrnl_id), ''MD5'')
     ...
     IF NOT rollback_all THEN
        vSql := vSql||' and exists (select 1 from slr_jrnl_headers_unposted sjhu
          where sjhu.jhu_jrnl_id in '||fnui_get_standard_hash_sql (journal_id_list)||'
          and sjhu.jhu_jrnl_id = standard_hash(to_char(gjlu.jlu_jrnl_hdr_id), ''MD5'')
          and sjhu.jhu_jrnl_status in (''E'',''V''))';
     END IF;
     ...
     and    jhu.jhu_jrnl_id = standard_hash(to_char(gjhu.jhu_jrnl_id), ''MD5'')
     and    jhu.jhu_jrnl_process_id = coalesce(:process_id, jhu.jhu_jrnl_process_id)
     and    jhu.jhu_jrnl_id in '||fnui_get_standard_hash_sql (journal_id_list);
     IF not rollback_all THEN
     ...
     select jhu_jrnl_id from slr_jrnl_headers_unposted where jhu_jrnl_id in '||fnui_get_standard_hash_sql (journal_id_list);
     IF NOT rollback_all THEN
     ...
     where jlu_jrnl_hdr_id in (
      ...
      (select p.jhu_jrnl_id from slr_jrnl_headers_unposted p where p.jhu_jrnl_id in '||fnui_get_standard_hash_sql (journal_id_list);
     IF NOT rollback_all THEN
     ...
     vSql := 'select jfa_jf_file_id
        ...
        where jfa_jh_jrnl_id in (select jhu_jrnl_id from slr_jrnl_headers_unposted  where jhu_jrnl_id in '||fnui_get_standard_hash_sql (journal_id_list);
     IF NOT rollback_all THEN
        vSql := vSql||' and jhu_jrnl_status in (''E'',''V'')';
     END IF;
     vSql := vSql||')';
  ...
  BEGIN
    ...
    EXECUTE IMMEDIATE
      ...
      and (exists (select 1 from slr_jrnl_headers_unposted sjhu
            where sjhu.jhu_jrnl_id = standard_hash(to_char(gjhu.jhu_jrnl_id), ''MD5'')
            and   sjhu.jhu_jrnl_status = ''W'')
          or exists (select 1 from slr_jrnl_headers sjh
            where sjh.JH_JRNL_ID = standard_hash(to_char(gjhu.jhu_jrnl_id), ''MD5'')
  ...
  BEGIN
    ...
    and jlu_jrnl_hdr_id in (select jhu_jrnl_id from slr_jrnl_headers_unposted
      where jhu_jrnl_id in '||fnui_get_standard_hash_sql (journal_id_list)||' and jhu_jrnl_status = ''W'')'
    USING status;
