...
---------------------------------------------------------------------------------
  ...
  e_lack_subpartition_error EXCEPTION;
...
-- In an effort to minimise changes to the package, rather than adding
-- the loop in to the main pPROCESS_SLR (as AG had in 4.10)
-- create a helper proc that does the loop and calls the proper pPROCESS_SLR!
...
BEGIN
  ...
  LOOP
    ...
    pPROCESS_SLR(v_epg_id, v_ent_rate_set);
  END LOOP;
...
AS
    ...
    s_proc_name VARCHAR2(80) := 'SLR_PKG.pPROCESS_SLR';
...
BEGIN
  ...
  END IF;
    ...
    -- ----------------------------------------------------------------------
	SLR_ADMIN_PKG.TruncatePartition(p_table_name=>'SLR_JRNL_LINES_UNPOSTED_ROLLBACK' ,p_part_value=>p_entity_proc_group);
    ...
    END;
     ...
     ----------------------------------------------------------------------------------------
      ...
      pg_process_state.log_proc(p_conf_group    => 'SLR',
                                p_stage         => 'slr_validate_journals_pkg.pValidateJournals',
                                p_process_id    => lv_process_id,
                                p_object_name   => 'SLR_JRNL_LINE_ERRORS',
                                p_epg_id        => p_entity_proc_group,
								p_rate_set      => gp_rate_set,
...
AS
    ...
    p_process NUMBER;
    s_proc_name       VARCHAR2(50) := 'SLR_PKG.pIMPORT_SLR_ALL_JRNLS';
...
-- -----------------------------------------------------------------------
  ...
  PROCEDURE pSet_Account_Process(p_process_id IN NUMBER,
                                 p_epg_id     IN slr_entity_proc_group.epg_id%TYPE) IS
    PRAGMA AUTONOMOUS_TRANSACTION;
  BEGIN
    EXECUTE IMMEDIATE ('UPDATE ' || slr_utilities_pkg.fhint(p_epg_id,'UPD_ACCOUNT_PROC') ||' fr_accounting_event SET ae_process_id =' || p_process_id ||' where  AE_EPG_ID= ''' || p_epg_id||'''');
    COMMIT;
...
AS
    ...
    s_proc_name CONSTANT VARCHAR2(80) := 'SLR_PKG.pIMPORT_SLR_JRNLS';
    lv_business_date SLR_ENTITIES.ENT_BUSINESS_DATE%TYPE;
    lv_user VARCHAR2(30);
	...
	v_table_name_err  user_tables.table_name%TYPE;
...
BEGIN
	...
	slr_post_journals_pkg.gp_business_date := lv_business_date;
    ...
    IF SLR_UTILITIES_PKG.fUserSubpartitionExists('FDR','FR_ACCOUNTING_EVENT_IMP', lv_subpartition_name) = FALSE THEN
        v_table_name_err :='FR_ACCOUNTING_EVENT_IMP';
       ...
       RAISE e_lack_subpartition_error;
    END IF;

    IF SLR_UTILITIES_PKG.fUserSubpartitionExists('SLR','SLR_JRNL_LINES_UNPOSTED', 'P'||p_entity_proc_group||'_SU') = FALSE THEN
        v_table_name_err :='SLR_JRNL_LINES_UNPOSTED';
		SLR_VALIDATE_JOURNALS_PKG.pWriteLogError(s_proc_name, 'SLR_JRNL_LINES_UNPOSTED',
            'Lack of subpartition in SLR_JRNL_LINES_UNPOSTED for EPG ' || p_entity_proc_group
            || ' subpartition ' || 'P'||p_entity_proc_group||'_SU', p_process_id,p_entity_proc_group);
        SLR_ADMIN_PKG.Error('ERROR in ' || s_proc_name || ': lack of subpartition in SLR_JRNL_LINES_UNPOSTED for EPG '
            || p_entity_proc_group || ' subpartition ' || 'P'||p_entity_proc_group||'_SU');
        RAISE e_lack_subpartition_error;
    END IF;
    ...
    pg_process_state.log_proc(p_conf_group    => 'SLR',
                              p_stage         => 'slr_client_procedures_pkg.pimport_slr_jrnls_su',
                              ...
                              p_sub_partition   => 'P'||p_entity_proc_group||'_SU',
                              p_start_dt      => sysdate,
                              p_status        => 'P');

    pg_process_state.log_proc(p_conf_group    => 'SLR',
                              p_stage         => 'slr_client_procedures_pkg.pimport_slr_jrnls_sw',
                              p_process_id    => p_process_id,
                              p_epg_id        => p_entity_proc_group,
							  p_business_date => slr_post_journals_pkg.gp_business_date,
							  p_rate_set      => gp_rate_set,
                              p_object_name   => 'SLR_JRNL_LINES_UNPOSTED',
                              p_tab_partition => 'P'||p_entity_proc_group,
                              p_sub_partition   => 'P'||p_entity_proc_group||'_SW',
                              p_start_dt      => sysdate,
                              p_status        => 'P');
    ...
    LOOP
      ...
      EXIT WHEN cValidateRows%NOTFOUND;
       ...
       pg_process_state.log_proc(p_conf_group   => 'SLR',
                                 p_stage        => 'slr_client_procedures_pkg.pimport_slr_jrnls_act_event',
                                 p_process_id   => p_process_id,
                                 p_epg_id        => p_entity_proc_group,
								 p_business_date => slr_post_journals_pkg.gp_business_date,
								 p_rate_set      => gp_rate_set,
                                 p_object_name  => 'FR_ACCOUNTING_EVENT_IMP',
                                 p_sub_partition  => SLR_UTILITIES_PKG.fSubpartitionName(p_entity_proc_group, v_posting_date),
                                 p_start_dt     => sysdate,
                                 p_status       => 'P');
    ...
    ELSE
		...
		pSet_Account_Process(p_process_id => p_process_id,
                             p_epg_id     => p_entity_proc_group );
...
EXCEPTION
    WHEN e_lock_acquire_error or e_internal_processing_error THEN
        -- error was logged before
     pg_process_state.Set_Process_Failed(p_process_id  => p_process_id,p_info=> SQLERRM);
     RAISE_APPLICATION_ERROR(-20001, 'Fatal error during import');
    WHEN e_lack_subpartition_error THEN
          RAISE_APPLICATION_ERROR(-20001, 'Lack of subpartition in '||v_table_name_err||' for EPG ' || p_entity_proc_group);
...
s_proc_name         VARCHAR2(80) := 'SLR_PKG.pUPDATE_SLR_SEGMENT_3';
...
s_proc_name         VARCHAR2(80) := 'SLR_PKG.pUPDATE_SLR_SEGMENT_4';
v_records_processed NUMBER(18);
...
s_proc_name         VARCHAR2(80) := 'SLR_PKG.pUPDATE_SLR_SEGMENT_5';
v_records_processed NUMBER(18);
...
s_proc_name         VARCHAR2(80) := 'SLR_PKG.pUPDATE_SLR_SEGMENT_7';
v_records_processed NUMBER(18);
...
s_proc_name         VARCHAR2(80) := 'SLR_PKG.pUPDATE_SLR_SEGMENT_8';
v_records_processed NUMBER(18);
...
s_proc_name VARCHAR2(80) := 'SLR_PKG.pUPDATE_SLR_CURRENCIES';
...
s_proc_name VARCHAR2(80) := 'SLR_PKG.pUPDATE_SLR_ACCOUNTS';
...
s_proc_name VARCHAR2(80) := 'SLR_PKG.pUPDATE_SLR_ENTITY_RATES';
lvBusinessDate DATE;
...
PROCEDURE pREC_AET_JRNLS (p_epg_id IN VARCHAR2,
    ...
    s_proc_name       VARCHAR2(50) := 'SLR_PKG.pREC_AET_JRNLS';
...
PROCEDURE pSLR_INTERNAL_REC (p_epg_id IN VARCHAR2,
    ...
    s_proc_name         VARCHAR2(50) := 'SLR_PKG.pSLR_INTERNAL_REC';
    lvBusinessDate      DATE;
...
PROCEDURE pROLL_ENTITY_DATE(p_entity in VARCHAR2, p_business_date in DATE DEFAULT NULL, p_update_end_date in CHAR DEFAULT 'Y') AS
    ...
    s_proc_name      VARCHAR2(50) := 'SLR_PKG.pROLL_ENTITY_DATE ';
    lvBusinessDate   DATE;
...
PROCEDURE pCHECK_JRNL_ERRORS(p_entity in VARCHAR2) IS
  v_count        NUMBER := 0;
  s_proc_name    VARCHAR2(80) := 'SLR_PKG.pCHECK_JRNL_ERRORS';
...
AS
    ...
    s_proc_name   VARCHAR2(50) := 'SLR_PKG.pINSERT_SLR_ENTITY_DAYS';
...
AS
    ...
    s_proc_name       VARCHAR2(60) := 'SLR_PKG.pINSERT_SLR_ENTITY_PERIODS';
...
AS
    ...
    s_proc_name       VARCHAR2(50) := 'SLR_PKG.pUPDATE_NON_WORKING_DAYS';
...
AS
    s_proc_name VARCHAR2(65) := 'SLR_PKG.pEntityBusinessDate';
BEGIN
...
EXCEPTION
    WHEN OTHERS THEN
        pr_error(slr_global_pkg.C_MAJERR, null, slr_global_pkg.C_TECHNICAL, 'SLR_PKG.pCustRunBalanceMovementProcess', NULL, NULL, NULL, 'SLR', 'PL/SQL', SQLCODE);
...
BEGIN
    ...
    LOOP
        ...
        pSLR_ENTITY_DAYS(V_ENTITY_SET,V_START_DATE,V_END_DATE,'O','AAH');
    END LOOP;
...
PROCEDURE pGENERATE_EBA_BOP_VALUES AS
    ...
    BEGIN
       ...
       MERGE /*+no_parallel*/ INTO SLR.SLR_EBA_BOP_AMOUNTS_TMP3 TMP3
               USING (
                        SELECT /*+no_parallel*/ JL_FAK_ID,
                               JL_EBA_ID,
                               JL_EFFECTIVE_DATE,
                               JL_PERIOD_YEAR,
                               JL_EPG_ID,
...
PROCEDURE pGENERATE_FAK_BOP_VALUES AS
  ...
  BEGIN
       ...
       /*Post the Y/E cleardown entries to the lastest record for each ID.  If no prior record, create one for December of the prior year*/
       MERGE /*+no_parallel*/ INTO SLR.SLR_FAK_BOP_AMOUNTS_TMP3 TMP3
               USING (
                        SELECT /*+no_parallel*/ JL_FAK_ID,
                               JL_EFFECTIVE_DATE,
                               JL_PERIOD_YEAR,
...
/* BEGIN FXRULE0 - MONTH-END FX REVALUATION */
  IF p_acc_basis = 'US_STAT'
    THEN pFX_REVAL_RULE0_USSTAT(V_FX_DT);
  ELSIF p_acc_basis = 'US_GAAP'
    THEN pFX_REVAL_RULE0_USGAAP(V_FX_DT);
  ELSIF p_acc_basis = 'UK_GAAP'
    THEN pFX_REVAL_RULE0_UKGAAP(V_FX_DT);
  END IF;

/* BEGIN FXRULE2 - MONTHLY FLOW FX REVALUATION */
  IF p_acc_basis = 'US_STAT'
    THEN pFX_REVAL_RULE2_USSTAT(V_FX_DT);
  ELSIF p_acc_basis = 'US_GAAP'
    THEN pFX_REVAL_RULE2_USGAAP(V_FX_DT);
  ELSIF p_acc_basis = 'UK_GAAP'
    THEN pFX_REVAL_RULE2_UKGAAP(V_FX_DT);
  END IF;
...
BEGIN
  ...
  pFX_REVAL_UPDATE_UNPOSTED( V_FX_RULE , PRATESET , V_BASIS , GPROCID , PENTPROCSET );
  commit;
