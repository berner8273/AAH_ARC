--------------------------------------------------------
--------------------------------------------------------
--  DDL for Package SLR_UTILITIES_PKG
--------------------------------------------------------
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE PACKAGE BODY "SLR_UTILITIES_PKG" AS
/*********************************************************************************
*
...
-- ----------------------------------------------------------------------------
    ...
    EXCEPTION
        WHEN OTHERS THEN
            ...
            slr_admin_pkg.debug(pLog => 'pRunValidateAndPost '||gv_msg , pLogExt => 'SQLERRM '||SQLERRM||' format_error_backtrace '||dbms_utility.format_error_backtrace);
            pg_process_state.Set_Process_Failed(p_process_id  => lv_process_id,p_info=> SQLERRM);
            raise_application_error(-20002, substr(gv_msg || '  ' || SQLERRM || ' ' ||dbms_utility.format_error_backtrace, 1, 2000));
...
-- -----------------------------------------------------------------
    ...
    PROCEDURE BackupData(p_subpartition  in  VARCHAR2, p_process_id    in  NUMBER,p_Epg_id in VARCHAR2, p_Wait in Varchar2 default 'N') IS
            PRAGMA AUTONOMOUS_TRANSACTION;
      BEGIN
             --SLR_ADMIN_PKG.TruncatePartition(p_table_name=>'SLR_JRNL_LINES_UNPOSTED_ROLLBACK' ,p_part_value=>p_Epg_id);
         if p_Wait = 'N' THEN
             EXECUTE IMMEDIATE 'insert  into slr_jrnl_lines_unposted_rollback  
          select '||SLR_UTILITIES_PKG.fHint(p_epg_id, 'RESET_FAILED_JLU')||'
            ...
            jlu_jrnl_rev_date, jlu_translation_date, jlu_period_month,
            jlu_period_year,  jlu_period_ltd, jlu_jrnl_internal_period_flag,
            jlu_jrnl_ent_rate_set,  jlu_type
             from slr_jrnl_lines_unposted subpartition ('||p_subpartition||')
            ...
            left join slr_entity_periods on jlu_entity = ep_entity and jlu_effective_date >= ep_cal_period_start
      and jlu_effective_date <= ep_cal_period_end and ep_period_type <> 0' using p_process_id;
    ...
    EXECUTE IMMEDIATE 'insert  into slr_jrnl_lines_unposted_rollback 
            ...
            from slr_jrnl_lines_waiting subpartition('||p_subpartition||')
...
-- --------------------------------------------------------------------
    ...
    AS
      ...
      BEGIN
        ...
        IF p_lock THEN
          EXECUTE IMMEDIATE 'lock table slr_jrnl_lines_waiting subpartition for (date '''||v_business_date_char||''','''||p_epg_id||''') in exclusive mode wait 600';
        END IF;
     ...
     WITH ses AS
          (SELECT substr(listagg(username, ',') within GROUP(ORDER BY audsid), 1, 100) AS username,
                  substr(listagg(sid, ',') within GROUP(ORDER BY audsid), 1, 100) AS sid,
                  substr(listagg(serial#, ',') within GROUP(ORDER BY audsid), 1, 100) AS serial,
                  substr(listagg(program, ',') within GROUP(ORDER BY audsid), 1, 100) AS program,
                  substr(listagg(userenv('SESSIONID'), ',') within GROUP(ORDER BY audsid), 1, 100) AS  sessionid
           FROM v$session
             WHERE audsid = userenv('SESSIONID') AND status = 'ACTIVE')
        SELECT
            username,
            sid,
            serial,
         ...
         INTO v_username,
              ...
              v_program,
              v_sessionid
      FROM ses WHERE rownum = 1;


        EXECUTE IMMEDIATE 'lock table slr_jrnl_lines_waiting subpartition for (date '''||v_business_date_char||''','''||p_epg_id||''') in exclusive mode wait 60';
        slr_admin_pkg.debug('lock table slr_jrnl_lines_waiting subpartition for (date '''||v_business_date_char||''','''||p_epg_id||''')
                         in exclusive mode wait 60  by user '||v_username||' v_sid '||v_sid||' v_serial '||v_serial||' v_program '||v_program||' v_sessionid '||v_sessionid);
      EXCEPTION
        WHEN e_missing_subpartition THEN
          paddjrnlwaitingsubpartition(TRUE);
        WHEN OTHERS THEN
          slr_admin_pkg.debug('FAILED lock table slr_jrnl_lines_waiting subpartition for (date '''||v_business_date_char||''','''||p_epg_id||''')
                           in exclusive mode wait 60  by user '||v_username||' v_sid '||v_sid||' v_serial '||v_serial||' v_program '||v_program||' v_sessionid '||v_sessionid);
          RAISE;
      ...
      BEGIN
        ...
        FOR recSubpartition IN (
          ...
          BackupData(p_subpartition=> recSubpartition.subpartition_name,p_process_id=>p_process_id,p_epg_id=>p_epg_id,p_Wait=>'Y') ;
      ...
      PROCEDURE pGetWaitingFirstPartitionDetails (
        p_partition_name OUT VARCHAR2,
        p_default_partition_date OUT SLR_JRNL_LINES_UNPOSTED.JLU_EFFECTIVE_DATE%TYPE
      )
      IS
        v_part_high_value VARCHAR2(2000);
      BEGIN
      EXECUTE IMMEDIATE 'SELECT part_high_value, partition_name
      FROM XMLTABLE(''/ROWSET/ROW'' PASSING -- DBMS_XMLGEN used as a workaround to extract part_high_value which is a long data type
        DBMS_XMLGEN.getXMLType(''select p.partition_position, p.high_value as part_high_value, p.partition_name
            from user_tab_partitions p
            where p.table_name=upper(''''SLR_JRNL_LINES_WAITING'''')
            order by partition_position'')
      COLUMNS
          ...
          partition_position VARCHAR2(4000) PATH ''PARTITION_POSITION/text()'',
          partition_name VARCHAR2(4000) PATH ''PARTITION_NAME/text()''
      ) X WHERE PARTITION_POSITION = 1' INTO v_part_high_value, p_partition_name;

        EXECUTE IMMEDIATE 'select ' || v_part_high_value || ' from dual' INTO p_default_partition_date;
    END pGetWaitingFirstPartitionDetails;

      PROCEDURE pRestoreJrnlWaitingPartition (
        p_partition_name IN VARCHAR2,
        p_default_partition_date IN SLR_JRNL_LINES_UNPOSTED.JLU_EFFECTIVE_DATE%TYPE,
        p_earliest_effective_date IN SLR_JRNL_LINES_UNPOSTED.JLU_EFFECTIVE_DATE%TYPE)
      IS
        v_sql_query CLOB;
        v_count PLS_INTEGER;
        v_earliest_bus_date DATE;
        v_end_date date;
        v_start_date DATE;
        v_current_date DATE;
        v_days PLS_INTEGER;
        v_table_ddl varchar2(10000) default 'ALTER TABLE SLR_JRNL_LINES_WAITING SPLIT PARTITION :pPartitionName INTO (';
        v_partition_ddl varchar2(10000) default 'PARTITION p:pPartitionName values less than (date '':p_Date''), ';
        v_last_partition_ddl varchar2(10000) default 'PARTITION :pPartitionName';
      BEGIN
        v_end_date := p_default_partition_date;

        EXECUTE IMMEDIATE q'[
        SELECT count(1)
        FROM USER_TAB_PARTITIONS
        WHERE TABLE_NAME = 'SLR_JRNL_LINES_WAITING'
        AND PARTITION_NAME = ']' || p_partition_name || '''' INTO v_count;

        IF V_COUNT = 1 THEN
          SELECT min(GP_TODAYS_BUS_DATE)
          INTO v_earliest_bus_date
          FROM FDR.FR_GLOBAL_PARAMETER,
               FDR.FR_LPG_CONFIG,
               SLR_JRNL_LINES_WAITING
          WHERE LC_LPG_ID = LPG_ID
          AND LC_GRP_CODE = JLW_ENTITY;

          v_start_date := least(nvl(p_earliest_effective_date, sysdate), nvl(v_earliest_bus_date, sysdate), sysdate) + 1;
          SELECT v_end_date - v_start_date INTO v_days FROM dual;
          IF (v_days > 0) THEN
            v_sql_query := REPLACE(v_table_ddl, ':pPartitionName', p_partition_name);
            FOR i IN 0..v_days-1
            LOOP
              SELECT v_start_date+i INTO v_current_date FROM dual;
              v_sql_query := v_sql_query || replace(replace(v_partition_ddl, ':pPartitionName', to_char(v_current_date, 'YYYYMMDD')), ':p_Date', to_char(v_current_date, 'YYYY-MM-DD'));
            END LOOP;
            v_sql_query := v_sql_query || replace(v_last_partition_ddl, ':pPartitionName', p_partition_name);
            v_sql_query := v_sql_query || ')';
            slr_admin_pkg.debug(pLog => 'pRestoreWaitingDatePartition query' , pLogExt => v_sql_query);
            EXECUTE IMMEDIATE v_sql_query;
          END IF;
        END IF;
      END pRestoreJrnlWaitingPartition;

      PROCEDURE pMoveErroredJournals
      IS
        v_earliest_effective_date SLR_JRNL_LINES_UNPOSTED.JLU_EFFECTIVE_DATE%TYPE;
      v_partition_name VARCHAR2(4000);
        v_default_partition_date SLR_JRNL_LINES_UNPOSTED.JLU_EFFECTIVE_DATE%TYPE;
      ...
      BEGIN
        ...
        ELSE
          p_use_headers := FALSE;
        END IF;
        -- Get default - first partition name and high value in the form of date
    pGetWaitingFirstPartitionDetails(v_partition_name, v_default_partition_date);

      pg_process_state.log_proc(p_conf_group    => 'SLR',
                                p_stage         => 'slr_utilities_pkg.pMoveErroredJournals',
                                p_process_id    => p_process_id,
                                p_object_name   => 'SLR_JRNL_LINES_WAITING',
                                p_epg_id        => p_epg_id,
        ...
        EXECUTE IMMEDIATE 'lock table slr_jrnl_lines_unposted subpartition for ('''||p_epg_id||''', ''E'') in exclusive mode wait 600';

        -- get the earliest JLU_EFFECTIVE_DATE going to be inserted to SLR_JRNL_LINES_WAITING, if older than the default partition high value
        v_sql_query := q'[
            select min(jlu_effective_date)
            from slr_jrnl_lines_unposted subpartition for (']' || p_epg_id || q'[', 'E')
                left join slr_entities on jlu_entity = ent_entity
                left join slr_entity_accounts on ea_entity_set = ent_accounts_set and ea_account = jlu_account
            ...
            where jlu_epg_id = ']'||p_epg_id||q'['
            and jlu_effective_date > date ']' || to_char(v_business_date, 'YYYY-MM-DD') || q'['
          and jlu_effective_date < date ']' || to_char(v_default_partition_date - 1, 'YYYY-MM-DD') || q'[']';

        slr_admin_pkg.debug(pLog => 'pMoveErroredJournals query' , pLogExt => v_sql_query);
        EXECUTE IMMEDIATE v_sql_query INTO v_earliest_effective_date;

        -- if JLU_EFFECTIVE_DATE older than the default partition high value found -
        -- - split default partition and create new one (with earlier date)
        IF v_earliest_effective_date IS NOT NULL THEN
            pRestoreJrnlWaitingPartition(v_partition_name, v_default_partition_date, v_earliest_effective_date);
        END IF;

        v_sql_query := 'insert /*+ append */ first
        ...
        slr_admin_pkg.debug(pLog => 'pMoveErroredJournals main query' , pLogExt => v_sql_query);
        EXECUTE IMMEDIATE v_sql_query using v_business_date, v_business_date;


          Commit;
           BackupData(p_subpartition=>'P'||p_epg_id||'_SE',p_process_id=>p_process_id,p_epg_id=>p_epg_id) ;
      ...
      PROCEDURE pMoveWaitingJournals
      IS
        v_earliest_effective_date SLR_JRNL_LINES_UNPOSTED.JLU_EFFECTIVE_DATE%TYPE;
      v_partition_name VARCHAR2(4000);
        v_default_partition_date SLR_JRNL_LINES_UNPOSTED.JLU_EFFECTIVE_DATE%TYPE;
    ...
    pGetWaitingFirstPartitionDetails(v_partition_name, v_default_partition_date);
        ...
        EXECUTE IMMEDIATE 'lock table slr_jrnl_lines_unposted subpartition for ('''||p_epg_id||''', ''W'') in exclusive mode wait 600';

        -- get the earliest JLU_EFFECTIVE_DATE going to be inserted to SLR_JRNL_LINES_WAITING, if older than the default partition high value
        v_sql_query := q'[
          select min(jlu_effective_date)
          from slr_jrnl_lines_unposted subpartition for (']' || p_epg_id || q'[', 'W')
            left join slr_entities on jlu_entity = ent_entity
            left join slr_entity_accounts on ea_entity_set = ent_accounts_set and ea_account = jlu_account
            left join slr_entity_periods on jlu_entity = ep_entity and jlu_effective_date >= ep_cal_period_start and jlu_effective_date <= ep_cal_period_end and ep_period_type <> 0
          where jlu_epg_id = ']' || p_epg_id || q'['
      ...
      and jlu_effective_date < date ']' || to_char(v_default_partition_date - 1, 'YYYY-MM-DD') || q'[']';
        ...
        slr_admin_pkg.debug(pLog => 'pMoveWaitingJournals query' , pLogExt => v_sql_query);
        EXECUTE IMMEDIATE v_sql_query INTO v_earliest_effective_date;
        ...
        slr_admin_pkg.debug(pLog => 'pMoveWaitingJournals main query' , pLogExt => v_sql_query);
        EXECUTE IMMEDIATE v_sql_query using v_business_date, v_business_date;

          Commit;
          BackupData(p_subpartition=>'P'||p_epg_id||'_SW',p_process_id=>p_process_id,p_epg_id=>p_epg_id);
    ...
    IF slr_post_journals_pkg.gp_business_date IS NULL THEN
      slr_post_journals_pkg.gp_business_date :=v_business_date;
    END IF;
    pg_process_state.log_proc(p_conf_group    => 'SLR',
                              p_stage         => 'slr_utilities.pResetFailedJournals',
                              p_process_id    => p_process_id,
                              p_epg_id        => p_epg_id,
                              p_business_date => slr_post_journals_pkg.gp_business_date,
							  p_rate_set      => slr_post_journals_pkg.gp_rate_set,
                              p_object_name   => 'SLR_JRNL_LINES_UNPOSTED',
                              p_tab_partition => slr_admin_pkg.GetPartitionName(p_table_name =>'SLR_JRNL_LINES_UNPOSTED_ROLLBACK',p_part_value=>p_epg_id),
                              p_start_dt      => sysdate,
                              p_status        => 'P');
 ...
 --        dbms_output.put_line(SQLERRM);
   --         dbms_output.put_line(dbms_utility.format_error_backtrace);
      slr_admin_pkg.debug(pLog => 'pRunValidateAndPost '||gv_msg , pLogExt => 'SQLERRM '||SQLERRM||' format_error_backtrace '||dbms_utility.format_error_backtrace);
            raise_application_error(-20002, substr(gv_msg || '  ' || SQLERRM || ' ' ||
...
BEGIN
    ...
    pg_process_state.log_proc(p_conf_group    => 'SLR',
                              ...
                              p_epg_id        => p_entity_proc_group,
                              p_business_date => slr_post_journals_pkg.gp_business_date,
							  p_rate_set      => slr_post_journals_pkg.gp_rate_set,
                              p_tab_partition => 'P'||p_entity_proc_group,
                              p_start_dt      => sysdate,
                              p_status        => 'P');
