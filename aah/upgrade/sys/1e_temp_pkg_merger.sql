...
sWhere := sWhere || ' AND NOT (map.le_cd = ''AGCRP'' and map_parent.le_cd = ''FSAUK'' and  fc_account like ''%-02'' and fc_segment_7 = ''CA'' and fc.fc_entity  in (''FSANY'', ''CARA1''))';   
...
-- FSAUK to AGCRP PGAAP that gets booked to FSANY
nScenerio := 7;
sWhere := ' AND map.le_cd = ''AGCRP'' and map_parent.le_cd = ''FSAUK'' and  fc_account like ''%-02'' and fc_segment_7 = ''CA'' and fc.fc_entity in (''FSANY'', ''CARA1'')';   
dbms_output.put_line(swhere);
sSQL := fBuildSql(sWhere,nScenerio);
execute immediate sSQL;
