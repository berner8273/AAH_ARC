...
BEGIN
    ...
    SELECT COUNT(*) INTO i FROM user_indexes WHERE index_name = 'BAK_XIF2GL_INTERFACE_JOURNAL_MAPPI';
    IF i = 0 THEN
        EXECUTE IMMEDIATE 'ALTER INDEX XIF2GL_INTERFACE_JOURNAL_MAPPI RENAME TO BAK_XIF2GL_INTERFACE_JOURNAL_MAPPI';
    END IF;    
...
BEGIN
    SELECT COUNT(*) INTO i FROM user_indexes WHERE index_name = 'XIF1GL_INTERFACE_JOURNAL_MAPPI';
    IF i = 0 THEN
        EXECUTE IMMEDIATE 'CREATE INDEX RDR.XIF1GL_INTERFACE_JOURNAL_MAPPI ON RDR.RR_GLINT_JOURNAL_MAPPING (RGJM_RGBC_ID)';
    END IF;

    SELECT COUNT(*) INTO i FROM user_indexes WHERE index_name = 'XIF2GL_INTERFACE_JOURNAL_MAPPI';
    IF i = 0 THEN
        EXECUTE IMMEDIATE 'CREATE INDEX RDR.XIF2GL_INTERFACE_JOURNAL_MAPPI ON RDR.RR_GLINT_JOURNAL_MAPPING (RGJM_RGJ_ID, RGJM_RGBC_ID)';
    END IF;
END;
/
