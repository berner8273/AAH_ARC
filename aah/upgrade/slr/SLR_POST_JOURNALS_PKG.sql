SET DEFINE OFF;
Prompt Package SLR_POST_JOURNALS_PKG;
...
-- -------------------------------------------------------------------------------
    ...
    -- creates table with all EBA daily balances using SLR_JRNL_LINES
	-- !!!DEPRECATED until next roadmap build. Logic nieed to take INTERNAL_PERIOD_FLAG into account!!!
  /*  PROCEDURE pCreateEbaBalancesFromLines
    (
        p_table_name IN VARCHAR2 -- name of balances table, which will be created
    );
*/
    -- do the same but using different algorithm (MODEL clause)
	-- !!!DEPRECATED until next roadmap build. Logic nieed to take INTERNAL_PERIOD_FLAG into account!!!
...
*/
    ...
    -- creates table with all FAK daily balances using SLR_JRNL_LINES
	-- !!!DEPRECATED until next roadmap build. Logic nieed to take INTERNAL_PERIOD_FLAG into account!!!
		...
		(
			...
			(
             ...
             AND ((JT_BALANCE_TYPE = ''20'' AND ent.ENT_ADJUSTMENT_FLAG = ''Y'') OR JT_BALANCE_TYPE <> ''20'')
				...
				GROUP BY EDB_BALANCE_DATE, EDB_EBA_ID, EDB_FAK_ID, EDB_BALANCE_TYPE, EDB_ENTITY, EDB_EPG_ID, EDB_PERIOD_MONTH, EDB_PERIOD_YEAR, EDB_PERIOD_LTD
				UNION ALL
				SELECT /*+ NO_MERGE */
