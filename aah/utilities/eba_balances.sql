...
--select EDB_BALANCE_TYPE,sum(EDB_TRAN_LTD_BALANCE) tran_sum, sum(EDB_BASE_LTD_BALANCE) base_sum, sum(EDB_LOCAL_LTD_BALANCE) local_sum
...
as (
    ...
    select db.EDB_BALANCE_TYPE||db.EDB_FAK_ID||db.EDB_EBA_ID||db.edb_balance_date 
        ...
        and m.mdate <= (gp.gp_todays_bus_date - arc.arct_archive_days )
                and db.EDB_BALANCE_TYPE = m.EDB_BALANCE_TYPE and db.EDB_EBA_ID = m.EDB_EBA_ID  )
group by edb_balance_date,EDB_BALANCE_TYPE,edb_entity 
order by edb_balance_date,EDB_BALANCE_TYPE,edb_entity 
