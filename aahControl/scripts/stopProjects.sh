...
function printLegend
{
    printDivider
    echo "*StopProjects                     :"
    echo "*StopProjects run datetime        : ${C_TIMESTAMP}"
    echo "*StopProjects aptitude host       : ${C_APTITUDE_HOST}"
    echo "*StopProjects aptitude bus port   : ${C_APTITUDE_BUS_PORT}"
    printDivider
}
...
function printCompletionMessage
{
    ...
    then
        echo "Projects stopped successfully."
    else
...
printLegend
java -server -cp ${C_PATH_TO_THIS_FOLDER}/startStopProjects.jar com.aptitudesoftware.appsvr.StopProjects ${C_APTITUDE_HOST} ${C_APTITUDE_BUS_PORT} ${C_TIMEOUT_MILLISECONDS} ${C_APTITUDE_SERVER_NAME}
return_code=$?
