...
inner join slr.v_slr_fxreval_rule1_eventunion r1u
    on ( fc.fc_segment_6   = r1u.execution_type
    and  fc.fc_segment_7   = r1u.business_type
    and  ec.ec_attribute_3 = r1u.premium_type
    and  ec.ec_attribute_4 = r1u.event_type)
...
where edb_balance_type  = 50
and FC.FC_ACCOUNT in 
  (
    select distinct lk_lookup_value2
    from fdr.fr_general_lookup
    where lk_lkt_lookup_type_code = 'FXREVAL_GL_MAPPINGS'
    and lk_match_key9 in ('FXRULE0','FXRULE2')
    and lk_match_key10 = 'Adjust'
    and lk_match_key1 = 'US_STAT'
  )
or EC.EC_ATTRIBUTE_4 in 
  (
  select distinct lk_match_key2
  from fdr.fr_general_lookup
  where lk_lkt_lookup_type_code = 'FXREVAL_GL_MAPPINGS'
  and lk_match_key9 =  'FXRULE1'
  )
;