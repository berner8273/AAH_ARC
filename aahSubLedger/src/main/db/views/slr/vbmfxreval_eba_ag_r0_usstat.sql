...
and fgl.lk_lookup_value1 = 'US_STAT')
inner join slr.v_slr_fxreval_rule0_accts accts
on ( fc.fc_account     = accts.source_account
 and fc.fc_segment_6   = accts.execution_type
 and fc.fc_segment_7   = accts.business_type
 and ec.ec_attribute_3 = accts.premium_type)
...
--on (event_hier.event_class = runval.event_class) -- need to figure out how to solve filter on event class without dropping off records from month-end GL accounts
where 
(edb_balance_type  = 50
and ea.ea_account in accts.source_account
and ea.ea_revaluation_flag = 'Y')
or ec.ec_attribute_4 in accts.fx_event_type;