...
select --Unposted SLR journals not GLINT processed
  ...
  from ( select
              ...
              , jt.ejt_madj_flag
           from
                slr.slr_jrnl_lines_unposted   sjl
      left join fdr.fr_general_lookup         fgl       on sjl.jlu_attribute_4         = fgl.lk_match_key1
                                                       and fgl.lk_lkt_lookup_type_code = 'EVENT_HIERARCHY'
      left join fdr.fr_gl_account             gl        on sjl.jlu_account             = gl.ga_account_code
      left join slr.slr_jrnl_headers_unposted jh        on jh.jhu_jrnl_id              = sjl.jlu_jrnl_hdr_id
      left join slr.slr_ext_jrnl_types        jt        on jt.ejt_type                 = jh.jhu_jrnl_type ) sjlu
 group by
       sjlu.jlu_entity
     , sjlu.jlu_segment_1
